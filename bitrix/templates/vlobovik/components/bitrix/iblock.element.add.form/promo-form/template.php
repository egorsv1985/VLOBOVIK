...
<form name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data">
	<div class="d-flex flex-column justify-content-center">
		<div class="fs-22 text-center lh-15">Оставьте заявку</div>
		<p class="fs-22 text-center lh-12 mb-3">
			<span class="fw-700">получите консультацию</span> бесплатно
		</p>
		<?= bitrix_sessid_post() ?>
				...
				<div class="form-group position-relative mb-3">
					...
					?>
						...
						case "N":
							...
							?>
								<input class="form-control form__input bg-white rounded-2 text-info p-3 h-100" type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" id="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" placeholder="" value="<?= $value ?>" required>
								<label for="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" class="form-label form__label fs-16 text-secondary position-absolute top-50"><? if (intval($propertyID) > 0) : ?>
								...
								if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime") : ?>
									<?
									$APPLICATION->IncludeComponent('bitrix:main.calendar', '', array('FORM_NAME' => 'iblock_add', 'INPUT_NAME' => "PROPERTY[" . $propertyID . "][" . $i . "]", 'INPUT_VALUE' => $value,), null,	array('HIDE_ICONS' => 'Y'));
									?>
									<small>
	...
	<button type="submit" name="iblock_submit" value="Получить" class="form__btn btn fs-18 fw-600 p-2 lh-15 btn-primary w-100 border-0 mb-3 text-white">
		Получить
	</button>
	<p class="fs-10 lh-15 text-center mb-0">
		Заполнение заявки вас ни к чему не обязывает. Ваши данные не будут
