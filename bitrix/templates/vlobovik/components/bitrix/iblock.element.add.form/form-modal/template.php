						...
						case "N":
							for ($i = 0; $i < $inputNum; $i++) {
								...
								<?if ($propertyID == 1 || $propertyID == 11):?>
									<input placeholder="<? if (intval($propertyID) > 0) : ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else : ?><?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID) ?><? endif ?>" class="form-control form__input bg-white rounded-2 text-info p-3 h-100" type="tel" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" id="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" placeholder="" value="<?= $value ?>" required>
								<?else:?>
									<input placeholder="<? if (intval($propertyID) > 0) : ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else : ?><?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID) ?><? endif ?>" class="form-control form__input bg-white rounded-2 text-info p-3 h-100" type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" id="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" placeholder="" value="<?= $value ?>" required>
								<?endif;?>
								<!--
								<label for="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" class="form-label form__label fs-16 text-secondary position-absolute top-50">
									<? if (intval($propertyID) > 0) : ?>
